...
type GymInterface interface {
	...
	// 添加门店
	CreateGym(gym *model.GymInfoModel) error
	// 更新场馆信息
...
type CoachInterface interface {
	...
	//新建教练
	CreateCoach(coach *model.CoachModel) error
...
type CoursePackageInterface interface {
	...
	//课包和场地解耦
	FindSamePackageNewRule(uid int64, coachId int) (*model.CoursePackageModel, error)
...
type CoursePackageSingleLessonInterface interface {
	...
	// 基于教练id分页获取单次课列表，按时间范围获取
	// status>0时按状态过滤，status<=0则获取全部状态
	// 游标翻页：首页传lastScheduleBegTs=0，后续传上一页最后一条的schedule_beg_ts
	GetSingleLessonListByCoachd(coachId int, monthBegTs int64, monthEndTs int64, status int, lastScheduleBegTs int64, limit int) ([]model.CoursePackageSingleLessonModel, error)
...
type AppointmentInterface interface {
	...
	// 兼容多门店
	SetAppointmentBookedNew(uid int64, appointmentID int, courseId int, gymId int) (error, model.CoachAppointmentModel)
	SetAppointmentUnAvailable(coachid int, appointmentID int, unavailableReason int) (error, model.CoachAppointmentModel)
...
// PreTrailManageInterface 体验课管理数据模型接口
type PreTrailManageInterface interface {
	// 添加体验课记录
	AddTrailManage(trail *model.PreTrailManageModel) error

	// 根据ID获取体验课记录
	GetTrailManageById(id int64) (*model.PreTrailManageModel, error)

	// 根据手机号获取体验课记录
	GetTrailManageByPhone(userPhone string) (*model.PreTrailManageModel, error)

	// 根据H5链接token获取体验课记录
	GetTrailManageByToken(h5LinkToken string) (*model.PreTrailManageModel, error)
	...
	// 根据门店ID获取体验课列表
	GetTrailManageListByGymId(gymId int, page, pageSize int) ([]model.PreTrailManageModel, error)
...
// 体验课管理数据模型实现
type PreTrailManageInterfaceImp struct{}
