...
func (imp *CoursePackageInterfaceImp) FindSamePackageNewRule(uid int64, coachId int) (*model.CoursePackageModel, error) {
	var err error
	var coursePackage = new(model.CoursePackageModel)
	cli := db.Get()
	err = cli.Table(course_package_tableName).Where("uid = ? AND coach_id = ?", uid, coachId).First(coursePackage).Error
	return coursePackage, err
}
...
func (imp *CoursePackageSingleLessonInterfaceImp) GetSingleLessonByAppointmentId(uid int64, appointmentID int) ([]model.CoursePackageSingleLessonModel, error) {
	...
	cli := db.Get()
	err = cli.Table(course_package_single_lesson_tableName).Select("lesson_id, package_id, appointment_id, appointment_id2, scheduled_by_coach, status, create_ts, is_confirm").
...
// GetSingleLessonListByCoachd 基于教练id分页获取单次课列表，按时间范围获取
// status>0时按状态过滤，status<=0则获取全部状态
// 游标翻页：首页传lastScheduleBegTs=0，后续传上一页最后一条的schedule_beg_ts
func (imp *CoursePackageSingleLessonInterfaceImp) GetSingleLessonListByCoachd(coachId int, monthBegTs int64, monthEndTs int64, status int, lastScheduleBegTs int64, limit int) ([]model.CoursePackageSingleLessonModel, error) {
	var err error
	var vecCoursePackageSingleLessonModel []model.CoursePackageSingleLessonModel
	cli := db.Get()
	tx := cli.Table(course_package_single_lesson_tableName).
		Where("coach_id = ? AND schedule_beg_ts >= ? AND schedule_beg_ts < ?", coachId, monthBegTs, monthEndTs)
	if status > 0 {
